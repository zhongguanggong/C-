...
#define Right 4
int map[100][39];
int path[1000];//0上，1下，2左，3右
int b[100][39];//标记是否访问过
...
{
    int c[4][2]={0,1,1,0,-1,0,0,-1}; //四组，对应向左，向右，向上，向下四个方向
    int i,j,t;
    //将方向打乱,四个方向的值随记交换
    for(i=0;i<4;i++)
    {
    ...
    for(i=0;i<4;i++){
        //printf("%d %d\n",c[i][0],c[i][1]);
        if(map[x+2*c[i][0]][y+2*c[i][1]]==Wall)//x,y设为是路，如果他的隔壁的隔壁是墙，就让他的隔壁是路
        {
            map[x+c[i][0]][y+c[i][1]]=Road;
            create(x+2*c[i][0],y+2*c[i][1]);//以他的隔壁的隔壁为切入点再递归
        }}
}
int get_key() //接收按键
{
...
//1上，2下，3左，4右
int find(int x,int y,int d){//自动寻路
    //printf("%d %d %d\n",x,y,map[x][y]);
    b[x][y] = 1;
    if(map[x][y]==End)
        return 1;
...
int main()
{
    ...
    for(i=0;i<=Height+1;i++)
        for(j=0;j<=Width+1;j++)
            if(i==0||i==Height+1||j==0||j==Width+1) //初始化迷宫//最边上是Height+1,但是只画 1<-->Height 内的
                map[i][j]=Road;
            else
                map[i][j]=Wall;

    //////////////////////////////
    create(2,2); //从随机一个点开始生成迷宫，该点行列都为偶数
    /////////////////////////////
